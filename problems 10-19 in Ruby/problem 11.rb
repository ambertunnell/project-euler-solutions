...
class GreatestProduct 
  ...
  def initialize 
    ...
    diagonal_right_combinations
    diagonal_left_combinations
    products
  end
...
def vertical_combinations
  i,j = 0,0 
  while i < 20
    if @grid[i+3] && @grid[i+3][j] 
      @combinations << [@grid[i][j], @grid[i+1][j], @grid[i+2][j],@grid[i+3][j]]
    else
      i+=1
      j = 0
      next 
...
def horizontal_combinations
  i,j = 0,0 
  while i < 20
    if @grid[i] && @grid[i][j+3]
      @combinations << [@grid[i][j], @grid[i][j+1], @grid[i][j+2],@grid[i][j+3]]
    else
      i+=1
      j = 0
      next 
...
def diagonal_right_combinations
  i,j = 0,0 
  while i < 20
    if @grid[i+3] && @grid[i+3][j+3]
      @combinations << [@grid[i][j], @grid[i+1][j+1], @grid[i+2][j+2],@grid[i+3][j+3]]
    else
      i+=1
      j = 0
      next 
...
def diagonal_left_combinations
  i,j = 0,0 
  while j < 20
    if @grid[i+3] && @grid[i+3][j-3]
      @combinations << [@grid[i][j], @grid[i+1][j-1], @grid[i+2][j-2],@grid[i+3][j-3]]
...
# print GreatestProduct.new.greatest_product
# answer = 70600674
